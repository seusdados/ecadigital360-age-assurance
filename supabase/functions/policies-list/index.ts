...
serve(async (req) => {
  ...
  try {
    ...
    let q = client
      .from('policies')
      .select(
        'id, tenant_id, slug, name, age_threshold, age_band_min, age_band_max, jurisdiction_code, method_priority_json, required_assurance_level, token_ttl_seconds, current_version, is_template, status',
      )
      .is('deleted_at', null)
      .eq('status', 'active');
