# HML — Segunda rotação da TENANT_API_KEY da application `dev-app`
...
> Data da rotação: 2026-05-09 20:23:46 UTC.
> Autor: Claude Code com autorização explícita do operador.
> Branch: `claude/hml-tenant-api-key-rotation-2026-05-09-2`.

## 1. Motivação — chave anterior comprometida

A `TENANT_API_KEY` rotacionada anteriormente em PR #65 (2026-05-09 12:19 UTC, hash começando `1624f0d2…`) **foi exposta em log/chat** durante a investigação dos smokes. Para preservar o princípio de menor exposição, o operador determinou rotação **adicional**, invalidando a chave exposta antes que possa ser usada por terceiros.

Este relatório registra **apenas o ato da rotação** — não inclui a raw key (que ficou exclusivamente com o operador) nem o snippet exato de log onde a exposição ocorreu (para evitar amplificar a exposição).

## 2. Modelo de segurança da rotação

Procedimento idêntico ao PR #65, executado novamente:
...
A raw key **anterior** (rotacionada em PR #65, hash começando `1624f0d2…`) foi exposta acidentalmente fora do cofre. Esta rotação a invalida: qualquer request com a raw antiga agora retorna 401 em `authenticateApiKey`.
...
- `RETURNING ...` → confirma execução com apenas campos não sensíveis.
...
| `api_key_prefix` | `ak_test_dp3nVZ…` | ✅ atualizado |
| `api_key_hash_first8` | `6a9edf77` | ✅ corresponde ao hash fornecido pelo operador |
| `tenant_id` | `019de8cd-d297-7db2-8f50-3ab5724d1645` | ✅ inalterado |
| `deleted_at` | `NULL` | ✅ ativo |
| `updated_at` | `2026-05-09 20:23:46.091143+00` | ✅ refletindo a rotação |
...
## 5. Cronologia das rotações da `dev-app` em HML
...
| 2 | 2026-05-09 20:23 | esta operação (UPDATE SQL) | `ak_test_dp3nVZ…` | `6a9edf77` | raw da rotação #1 exposta em log/chat |
...
- ✅ Apenas: 1 UPDATE atômico em 3 colunas (`api_key_hash`, `api_key_prefix`, `updated_at`) de 1 linha em `public.applications` em HML.
...
Com a nova `TENANT_API_KEY` na sua posse, e os scripts atualizados em `main` (PR #67):
...
Cuidados de segurança operacional para evitar exposição da chave:
...
- Se a chave **vazar de novo**, repetir o procedimento §3 (gerar nova → enviar HASH+PREFIX → autorizar UPDATE).

## 8. Hashes de auditoria

| Item | Valor (truncado) | Significado |
|---|---|---|
| Hash novo (first 8) | `6a9edf77` | SHA-256 da nova raw key — gravado em DB |
| Prefix novo | `ak_test_dp3nVZ…` | Display em painel; primeiros 14 chars + `…` |
| Hash invalidado pela rotação anterior (PR #65) | `1624f0d2` | exposto em log/chat — agora **invalidado** |
| `updated_at` desta rotação | `2026-05-09 20:23:46.091143+00` | Timestamp |
