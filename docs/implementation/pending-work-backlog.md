...
| AK-P0-04 | Segurança | Testes RLS cross-tenant automatizados | M | nenhum | `pnpm test:rls` cobre 6+ cenários de breakout; CI bloqueia merge em regressão | `supabase/functions/_tests/`, `turbo.json` | técnico | open |
| AK-P0-05 | Privacidade | Privacy guard automated payload scan em CI | S | privacy-guard.ts (já feito) | CI roda ferramenta de fuzz contra Edge Function responses em staging; nenhum forbidden key em 100 sessões | privacy-guard tests + CI step | técnico | open |
